		...
		private void execute_client_registration_job() throws CoseException, IOException{
			if(this.parameters.server_metadata.registration_endpoint != null &&
				...
				case coap_dtls_rpk:
					...
					//<short,cborstring>
					params.Add(CLIENT_METADATA.Cose_Key.getIdValue(), CBORObject.FromObject(this.parameters.client_metadata.cose_key));
					//<short,cborstring>
					params.Add(CLIENT_METADATA.Contact.getIdValue(), CBORObject.FromObject(this.parameters.client_metadata.contact));
						...
						if(result.getType().equals(CBORType.Map)) {
							for (CBORObject key : result.getKeys()) {
								...
								case Cose_Key:
									rvalue = result.get(key);
									if(rvalue.getType().equals(CBORType.TextString)) {
										this.parameters.client_metadata.cose_key = rvalue.AsString();
										this.client_registration_result.append("cose_key:").append(this.parameters.client_metadata.cose_key).append("\n");
									}else {
										this.parameters.client_metadata.cose_key = null;
										this.client_registration_result.append("cose_key:").append(this.parameters.client_metadata.cose_key).append("\n");
									}
									break;
								case Contact:
									rvalue = result.get(key);
									if(rvalue.getType().equals(CBORType.TextString)) {
										this.parameters.client_metadata.contact = rvalue.AsString();
										this.client_registration_result.append("contact:").append(this.parameters.client_metadata.contact).append("\n");
									}else {
										this.parameters.client_metadata.contact = null;
										this.client_registration_result.append("contact:").append(this.parameters.client_metadata.contact).append("\n");
									}
									break;
								case Client_Id:
