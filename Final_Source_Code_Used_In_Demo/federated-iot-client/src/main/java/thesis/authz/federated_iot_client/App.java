...
public class App 
{
	...
	public static void main( String[] args ) throws Exception, Exception
	{
		...
		String keyStorelocation = cconfig.getString("keystorelocation");
		String trustStorelocation = cconfig.getString("truststorelocation");
		String keyStorepassword = cconfig.getString("keystorepass");
		String trustStorepassword = cconfig.getString("truststorepass");
		String alias = cconfig.getString("alias");
		...
		Certificate[] len = keyStore.getCertificateChain(alias);
		System.out.println("Printing certificates " + len[0].toString() + " " +len[1].toString());
		builder.setIdentity((PrivateKey)keyStore.getKey(alias, keyStorepassword.toCharArray()),
		...
		if(RSscopes != null) {
			...
			CBORObject target = CBORObject.FromObject(AS_id);
			CBORObject audience = CBORObject.FromObject(RS_id);
			CBORObject scopes = CBORObject.FromObject(RSscopes);
			...
			params.put(Constants_ma.TAR, target);
			params.put(Constants.AUD, audience);
			params.put(Constants.SCOPE, scopes);
			...
			params.put(Constants.GRANT_TYPE, CBORObject.FromObject(Constants_ma.ASYMMETRIC));
			...
			while(true) {
				System.out.println(" Enter \n 1. 'status' to get the status of the light \n 2. 'fire' to trigger emergency \n 3. 'extinguished' to trigger fire extinguished \n"
						+ "4. 'exit' to stop smoke detector");
				...
				case "status":
					...
					case UNAUTHORIZED:
						...
						//Runtime.getRuntime().exec("./speech.sh " + " Received Unauthorized response. You are Unauthorized to access this resource");
						Runtime.getRuntime().exec("echo Received Unauthorized response. You are Unauthorized to access this resource | festival --tts");
						break;
					case FORBIDDEN:
						System.out.println("Received FORBIDDEN response");
						res = CBORObject.DecodeFromBytes(response.getPayload());
						System.out.println("Printing Received payload " + res.toString());
						//Runtime.getRuntime().exec("./speech.sh " + " Received Forbidden response. You are Forbidden to access this resource");
						Runtime.getRuntime().exec("echo Received Forbidden response. You are Forbidden to access this resource | festival --tts");
						break;
					...
					case METHOD_NOT_ALLOWED:
						...
						res = CBORObject.DecodeFromBytes(response.getPayload());
						System.out.println("Printing Received payload " + res.toString());
						//Runtime.getRuntime().exec("./speech.sh " + " Received Method Not Allowed response. You are Unauthorized to access this resource");
						Runtime.getRuntime().exec("echo Received Method Not Allowed response. You are Unauthorized to access this resource | festival --tts");
						break;
					case CONTENT:
						System.out.println("Received CONTENT response");
						String con = response.getResponseText();
						System.out.println("Printing Received Content " + con);
						//Runtime.getRuntime().exec("./speech.sh " + con);
						Runtime.getRuntime().exec("echo '"+con+"' | festival --tts");
						break;
				...
				case "fire": 
					...
					response = client.put("true", MediaTypeRegistry.TEXT_PLAIN);
					c = response.getCode();
					switch(c) {
					case UNAUTHORIZED:
						System.out.println("Received UNAUTHORIZED response");
					...
					case CONTENT:
						...
						//Runtime.getRuntime().exec("./speech.sh " + "Fire. Fire. Fire. Please follow the lights to the nearest exit.");
						Runtime.getRuntime().exec("/usr/bin/mplayer Fire.mp3");
						break;
				...
				case "extinguished": 
					System.out.println(" ##### SENDING RESOURCE ACCESS PUT REQUEST TO ##### ");
					response = client.put("false", MediaTypeRegistry.TEXT_PLAIN);
					c = response.getCode();
					switch(c) {
					case UNAUTHORIZED:
						System.out.println("Received UNAUTHORIZED response");
					...
					case CONTENT:
						...
						String con = response.getResponseText();
						System.out.println(con);
						//Runtime.getRuntime().exec("./speech.sh " + " Fire is Extinguished.");
						Runtime.getRuntime().exec("/usr/bin/mplayer Extinguished.mp3");
						break;
