...
public class Client_Params {
	...
	public class Client_Metadata{
		...
		public List<JSONObject> cose_key; //type array of JSONObject
		...
		public Client_Metadata() {
			...
			this.cose_key = new ArrayList<JSONObject>();
		}
	}
				...
				case fetch_as_config:
					...
					if(key_value instanceof Boolean) {
						if((Boolean)key_value) { //If discovery is set to true
							...
							value = fetch_as_config_result.get("authorization_mode");
							if(value instanceof String) 
								this.server_metadata.auth_mode = value.toString();
							else{
							...
							if(value instanceof JSONArray) {
								...
								for(int i=0; i<j.length(); i++) {
									...
									if(Obj instanceof JSONObject) {
										this.client_metadata.cose_key.add((JSONObject) Obj);
									}
								}
							}
							else {
								LOGGER.log(Level.INFO, "Invalid cose_key parameter value");
								return;
							}
						}
						else {			
