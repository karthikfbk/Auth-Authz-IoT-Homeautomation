...
//	}
	...
	public static void main( String[] args ) throws Exception, Exception
	{
		...
		if(discovered_links != null && discovered_links.size() > 0) {
			for (WebLink s : discovered_links) {
				if(s.getURI().equals("/Info")) {
					...
					RSinfoendpoint = s.getAttributes().getAttributeValues("rsinfoendpoint").get(0);
		...
		System.out.println(" RSinfoendpoint " + RSinfoendpoint);
		System.out.println(" AS_id " + AS_id);
	
		System.out.println(" AS_tokenendpoint " + AS_tokenendpoint);
		/*
		 * RS INFORMATION FETCH
		 */
		int count = 0;
		while(count < retry_counter) {
		if(RSinfoendpoint != null) {

			// Do a get of rsinfo for max of 3 times. If there is certificate error retry
			client.setURI(RSinfoendpoint);
			System.out.println(" GETTING RSINFO ...... ");
			CoapResponse response = client.get();

			if(response == null) {
				//assume that the hand shake has terminated as client cannot validate server certificate.
				//now send a connect request and retry
				System.out.println("Sending Connect Request because received null response");

				/*
				 * CONNECT
				 */

				JSONObject connect_request = (JSONObject) cconfig.get("CONNECT_REQUEST");
				//Send Connect Request
				Map<Short, CBORObject> params = new HashMap<>();
				params.put(Constants_ma.REQUEST_TYPE, CBORObject.FromObject(Constants_ma.CONNECT));
				params.put(Constants_ma.TAR, CBORObject.FromObject(AS_id));									
				client.setURI(connect_request.getString("uri"));

				if(connect_request.getBoolean("execute")) {
					System.out.println(" ##### SENDING CONNECT REQUEST TO ##### " + connect_request.getString("uri"));
					...
					System.out.println("Receided Connect Response " + map);
				...
				//response is not null. we got rsinfo
				CBORObject res = CBORObject.DecodeFromBytes(response.getPayload());
				Map<Short, CBORObject> map = Constants.getParams(res);
				System.out.println("Received RSInfo Response " + map);
				
				CBORObject target = map.get(Constants_ma.AS_ID);
				CBORObject audience = map.get(Constants_ma.RS_ID);
				CBORObject scopes = map.get(Constants.SCOPE);
				...
				params.put(Constants_ma.PROVISION_TYPE, CBORObject.FromObject(Constants_ma.ASYMMETRIC));
