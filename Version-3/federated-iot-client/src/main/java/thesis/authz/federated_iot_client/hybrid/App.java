...
public class App 
{
	...
	public static void main( String[] args ) throws Exception, Exception
	{
		...
		if(discovered_links != null && discovered_links.size() > 0) {
			for (WebLink s : discovered_links) {
				if(s.getURI().equals("/Info")) {
					...
					infoendpoint = s.getAttributes().getAttributeValues("infoendpoint").get(0);
		...
		System.out.println(" infoendpoint " + infoendpoint);
		...
		while(count < retry_counter) {
			if(infoendpoint != null) {
				...
				client.setURI(infoendpoint);
				System.out.println(" FETCHING INFO about Discovered RS ...... ");
				Map<Short, CBORObject> params1 = new HashMap<>();
				params1.put(Constants_ma.RS_ID, CBORObject.FromObject(RS_id));
				params1.put(Constants_ma.CAS_ID, CBORObject.FromObject(cas_name));

				CoapResponse response = client.post(Constants.getCBOR(params1).EncodeToBytes(), 
				...
				if(response == null) {
					...
					CBORObject res = CBORObject.DecodeFromBytes(response.getPayload());
					Map<Short, CBORObject> map = Constants.getParams(res);
					System.out.println("Received Fetch info Response " + map);

					CBORObject rstoken = map.get(Constants_ma.ID_TOKEN);
					...
					params.put(Constants_ma.PROVISION_TYPE, CBORObject.FromObject(Constants_ma.HYBRID));
					params.put(Constants_ma.TAR, CBORObject.FromObject(AS_id));
					params.put(Constants.AUD, CBORObject.FromObject(RS_id));
					params.put(Constants_ma.ID_TOKEN, rstoken);
					...
					if(access_token_request.getBoolean("execute")) {
						...
						dis_client.setURI(cconfig.getString("authz_info_uri"));
